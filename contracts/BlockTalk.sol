...
contract BlockTalkContract {
    uint public constant REWARD_DIFF = 5;
    ...
    function addTweet(
        ...
        users[msg.sender].tweetId.push(tweetId);
    ...
    function myTweets() external view returns (TweetView[] memory) {
        uint count = 0;
        for (uint i = 0; i < tweets.length; i++) {
    ...
    function addTweetWithHashtags(
        string memory title,
        string memory tweetText,
        string[] memory tags
    ) external {
        addTweet(title, tweetText, false);
        for (uint i = 0; i < tags.length; i++) {
            hashtags[tags[i]].push(tweets.length - 1);
        }
    }
    ...
    function getTrendingTweets() external view returns (TweetView[] memory) {
        uint count;
        for (uint i = 0; i < tweets.length; i++) {
            if (tweets[i].upvote > 15) count++;
        }

        TweetView[] memory result = new TweetView[](count);
        uint index;
        for (uint i = 0; i < tweets.length; i++) {
            if (tweets[i].upvote > 15) {
                Tweet storage tweet = tweets[i];
                result[index] = TweetView(
                    tweet.id,
                    tweet.username,
